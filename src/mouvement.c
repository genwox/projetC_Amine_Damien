...
{
    /* Initialisation lazy des états véhicules */
    if (!states_initialized) {
        for (int i = 0; i < MAX_VOITURES; i++) {
            states[i].vehicule = NULL;
            states[i].target_place = -1;
            states[i].lane_lock_counter = 0;
        }
        states_initialized = 1;
    }

    /* Recherche état existant */
    for (int i = 0; i < MAX_VOITURES; i++) {
        if (states[i].vehicule == v) {
            return &states[i];
        }
    }

    /* Création nouvel état si slot libre */
    for (int i = 0; i < MAX_VOITURES; i++) {
        if (!states[i].vehicule) {
            states[i].vehicule = v;
            return &states[i];
        }
    }
...
{
    VehiculeState *s = get_state(v);
    if (s) {
        s->lane_lock_counter = TURN_LOCK_DURATION;
    }
}
...
{
    VehiculeState *s = get_state(v);
    return s ? s->lane_lock_counter > 0 : 0;
}

static int obtenir_target(VEHICULE *v)
{
    VehiculeState *s = get_state(v);
    return s ? s->target_place : -1;
}
...
{
    /* Compteurs de flèches par direction: N, S, E, O */
    counts[0] = 0;
    counts[1] = 0;
    counts[2] = 0;
    counts[3] = 0;
    ...
    for (int y = cy - R; y <= cy + R; y++) {
        for (int x = cx - R; x <= cx + R; x++) {
            ...
            if (!est_fleche_circulation(plan->plan_statique[y][x])) {
                continue;
            }
...
{
    int nord = est_cellule_roulable(plan, cx, cy - 1);
    int sud = est_cellule_roulable(plan, cx, cy + 1);
    int est = est_cellule_roulable(plan, cx + 1, cy);
    int ouest = est_cellule_roulable(plan, cx - 1, cy);
...
static char detecter_fleche_proche(PlanParking *plan, int cx, int cy)
{
    ...
    if (!trouver_cellule_dans_zone(plan, cx, cy, 3, est_fleche_parking, &fx, &fy)) {
        return '\0';
    }
...
/* Détermine si le véhicule doit recalculer sa direction selon contexte */
static int doit_recalculer_direction(VEHICULE *vehicule, PlanParking *plan, int fleches_zone[4], int target_x, int target_y)
{
    ...
    /* Lane-lock actif bloque le recalcul */
    if (lane_lock_bloque_changement(vehicule, plan, cx, cy, dir)) {
        return 0;
    }

    /* Flèche immédiate force le recalcul */
    if (est_fleche_circulation(plan->plan_statique[cy][cx]) || detecter_fleche_proche(plan, cx, cy)) {
        return 1;
    }

    /* Flèches perpendiculaires à la direction suggèrent virage */
    int fp = (dir == 'N' || dir == 'S') ? fleches_zone[2] + fleches_zone[3] : fleches_zone[0] + fleches_zone[1];
    if (fp > 0) {
        return 1;
    }

    /* Obstacle devant force le recalcul */
    int dx = 0;
    int dy = 0;
    obtenir_delta_direction(dir, &dx, &dy);
    ...
    if (!peut_deplacer_sur_allee(vehicule, plan, nx, ny)) {
        if (nx < 0 || ny < 0 || nx + w > plan->largeur || ny + h > plan->hauteur) {
            return 1;
        }
        ...
        if (est_dans_limites(plan, ncx, ncy) && est_cellule_passable(plan->plan_statique[ncy][ncx])) {
            return 0;
        }
    ...
    if (compter_voisins_roulables(plan, cx, cy) >= 3) {
        return 1;
    }

    /* Cible éloignée dans direction opposée */
    if (target_x >= 0 && target_y >= 0) {
        int dcx = target_x - cx;
        int dcy = target_y - cy;

        if ((dir == 'N' && dcy > 5) || (dir == 'S' && dcy < -5) ||
            (dir == 'E' && dcx < -5) || (dir == 'O' && dcx > 5)) {
            return 1;
        }
    }
...
/* Calcule scores directionnels avec bonus de zones (pathfinding) */
...
{
    if (!plan || !scores || !meilleur_score || !dir_meilleure) {
        return;
    }

    /* Compter les flèches dans la zone */
    int fleches[4];
    compter_fleches_zone(plan, cx, cy, RAYON_SCAN, fleches);

    /* Compter les cases libres dans chaque direction */
    int passes[4];
    passes[0] = compter_passes(plan, cx, cy, 'N');
    passes[1] = compter_passes(plan, cx, cy, 'S');
    passes[2] = compter_passes(plan, cx, cy, 'E');
    passes[3] = compter_passes(plan, cx, cy, 'O');

    /* Score de base: 10 points par case libre + 3 points par flèche */
    for (int i = 0; i < 4; i++) {
        scores[i] = 10 * passes[i] + 3 * fleches[i];
    }

    /* Bonus vers la cible si connue */
    if (target_x >= 0 && target_y >= 0) {
        int dx = target_x - cx;
        int dy = target_y - cy;
    ...
    /* Bonus par zones géographiques du parking */
    for (int z = 0; z < 3; z++) {
        const ZoneBonus *zb = &ZONES[z];

        if (cy < zb->y_min || cy > zb->y_max) {
            continue;
        }

        if (zb->x_threshold > 0 && cx <= zb->x_threshold) {
            for (int i = 0; i < 4; i++) {
                if (passes[i] > 0) {
                    scores[i] += zb->bonus[i];
                }
            }
        } else if (zb->x_min > 0 && cx >= zb->x_min && cx <= zb->x_max) {
            for (int i = 0; i < 4; i++) {
                if (passes[i] > 0) {
                    scores[i] += zb->bonus[i];
                }
            }
        } else if (zb->x_threshold < 0 && zb->x_min < 0) {
            for (int i = 0; i < 4; i++) {
                if (passes[i] > 0) {
                    scores[i] += zb->bonus[i];
                }
...
static int a_parking_libre_proche(PlanParking *plan, int cx, int cy, char dir)
{
    ...
    if (sens_fleche(plan->plan_statique[fy][fx]) != dir) {
        return 0;
    }
...
/* Empêche changements de voie brusques (lane-keeping) */
static char valider_direction_lane_keeping(VEHICULE *vehicule, PlanParking *plan, int cx, int cy, char old, char new)
{
    ...
    /* Bloquer changements latéraux hors intersections */
    int est_lateral = ((old == 'N' || old == 'S') && (new == 'E' || new == 'O')) ||
                      ((old == 'E' || old == 'O') && (new == 'N' || new == 'S'));

    int nb_voisins = compter_voisins_roulables(plan, cx, cy);
    int est_intersection = nb_voisins >= 3 || est_fleche_circulation(plan->plan_statique[cy][cx]);

    if (est_lateral && old_safe && !est_intersection && !a_parking_libre_proche(plan, cx, cy, new)) {
        return old;
    }

    /* Bloquer demi-tours */
    int est_demi_tour = (old == 'N' && new == 'S') || (old == 'S' && new == 'N') ||
                        (old == 'E' && new == 'O') || (old == 'O' && new == 'E');
...
static char choisir_direction_stable(VEHICULE *vehicule, PlanParking *plan, int target_x, int target_y)
{
    ...
    /* Suivre les flèches simples immédiatement */
    if (c_actuel == L'←' || c_actuel == L'→' || c_actuel == L'↑' || c_actuel == L'↓') {
        char nouvelle_dir = sens_fleche(c_actuel);
        return nouvelle_dir;
    }

    /* Rechercher flèches de circulation proches */
    ...
    /* Vérifier flèche de parking proche */
    char dp = forcer_direction_si_fleche_parking(vehicule, plan, cx, cy);
    if (dp) {
        return dp;
    }

    /* Calcul pathfinding par scores */
    int scores[4];
    int ms = 0;
    char dm = 'N';
    ...
    /* Privilégier direction actuelle si score proche */
...
void suivre_fleches(VEHICULE *vehicule, PlanParking *plan)
{
    ...
    /* Recherche flèche la plus proche (distance Manhattan) */
    int md = 999;
    int mi = -1;
    ...
    if (mi >= 0) {
        ...
        if (!de || vehicule->direction == de) {
            vehicule->direction = plan->fleches[mi].direction_sortie;
        }
    }
}
void deplacer_vehicule(VEHICULE *vehicule, PlanParking *plan)
{
    ...
    int nx = vehicule->posx + dx * vehicule->vitesse;
    int ny = vehicule->posy + dy * vehicule->vitesse;

    if (peut_deplacer(vehicule, plan, nx, ny)) {
        vehicule->posx = nx;
        vehicule->posy = ny;
        return;
    }

    /* Tentative dans autres directions si bloqué */
    char dirs[] = {'N', 'S', 'E', 'O'};
    for (int i = 0; i < 4; i++) {
        if (dirs[i] == vehicule->direction) {
            continue;
        }

        int tx = vehicule->posx;
        int ty = vehicule->posy;
        int tdx = 0;
        int tdy = 0;

        obtenir_delta_direction(dirs[i], &tdx, &tdy);
        tx += tdx * vehicule->vitesse;
        ty += tdy * vehicule->vitesse;

        if (peut_deplacer(vehicule, plan, tx, ty)) {
            vehicule->direction = dirs[i];
            orienter_carrosserie(vehicule);
            vehicule->posx = tx;
            vehicule->posy = ty;
            return;
...
/* Recherche place libre la plus proche (distance Manhattan) */
...
/* Détecte flèche parking proche et téléporte véhicule à la place */
static int tenter_parking_automatique(VEHICULE *vehicule, PlanParking *plan)
{
    ...
    /* Recherche flèche de parking proche */
    int fleche_x = -1;
    int fleche_y = -1;
    ...
    /* Recherche place libre sur la ligne */
    int place_trouvee = -1;
    int dist_min = 99999;
...
void deplacer_vers_place(VEHICULE *vehicule, PlanParking *plan, int index_place)
{
    ...
    /* Déplacement horizontal prioritaire */
    ...
    /* Déplacement vertical si horizontal bloqué */
...
void deplacer_vehicule_parking_auto(VEHICULE *vehicule, PlanParking *plan, l_car *tous_vehicules)
{
    ...
    /* Recherche nouvelle cible si actuelle invalide */
    ...
    /* Tentative parking automatique si flèche détectée */
...
static int vehicule_a_sortie(VEHICULE *vehicule, PlanParking *plan)
{
    ...
    /* Vérification sur marqueur sortie 'S' */
    for (int dy = 0; dy < hauteur; dy++) {
        for (int dx = 0; dx < largeur; dx++) {
    ...
    /* Vérification proximité zone sortie */
...
int deplacer_tous_vehicules(l_car *vehicules, PlanParking *plan)
{
    ...
    /* Décrementer les compteurs de verrouillage de voie */
    if (states_initialized) {
        for (int i = 0; i < MAX_VOITURES; i++) {
            if (states[i].vehicule && states[i].lane_lock_counter > 0) {
                states[i].lane_lock_counter--;
            }
        }
    }

    /* Détecter véhicules à la sortie */
    ...
    /* Déplacer tous les véhicules actifs */
