...
{
    if (!vehicule || vehicule->etat != '1') return 0;
    return position_valide(vehicule, plan, nouveau_x, nouveau_y, 0);
}
int peut_deplacer_sur_allee(VEHICULE *vehicule, PlanParking *plan, int nouveau_x, int nouveau_y)
{
    return position_valide(vehicule, plan, nouveau_x, nouveau_y, 1);
}
